...
//        $lowers_id = $this->container->get(UserRelation)
    ...
    public function create(array $params)
    {
        ...
        if ($this->container->get(MemberDAO::class)->getIpUserCount($ip) >= 2) {
            $this->error('logic.IP_REGISTER_ERROR');
        }

        // 获取会员等级
        $user_level = $this->container->get(UserLevelDAO::class)->findByLevel(0);
        if (!$user_level) {
            $this->error('logic.LEVEL_NOT_FOUND');
        }
        ...
        try {
            ...
            $this->container->get(UserMemberDAO::class)->updateOrCreate([
                'user_id' => $user->id,
                'level' => $user_level->level,
                'effective_time' => -1
            ]);
    ...
    public function update(array $params)
    {
        ...
        $user_info_data = [];
        if (!empty($params['user_info']['bank_name'])) {
            $user_info_data['bank_name'] = $params['user_info']['bank_name'];
        }

        if (!empty($params['user_info']['bank_address'])) {
            $user_info_data['bank_address'] = $params['user_info']['bank_address'];
        }

        if (!empty($params['user_info']['name'])) {
        ...
        if (!empty($params['user_info']['bank_code'])) {
            $user_info_data['bank_code'] = $params['user_info']['bank_code'];
        }
        
         if (!empty($params['user_info']['rank'])) {
            $user_info_data['rank'] = $params['user_info']['rank'];
        }
