...
class UserLevelService extends Base
{
    ...
    public function create(array $params)
    {
        ...
        try {
            ...
            $level = $this->container->get(UserLevelDAO::class)->create([
                ...
                'duration' => $duration,
                'task_val' => $params['task_val'],
                'withdrawal_times' => $params['withdrawal_times'],
            ]);
    ...
    public function edit(int $id, array $params)
    {
        ...
        // 判断等级是否重复
        if ($this->container->get(UserLevelDAO::class)->findByLevel((int)$params['level'])->id !== $id) {
            $this->formError([
                'level' => 'logic.LEVEL_ALREADY_EXISTS'
            ]);
        }
        ...
        try {
            ...
            $level->task_val = $params['task_val'];
            $level->withdrawal_times = $params['withdrawal_times'];
            $level->save();
    ...
    public function rechargeLevel(int $user_id, int $level, string $recharge_remark, $trade_no, int $channel)
    {
        ...
        try {
            ...
            foreach ($country_list as $key => $lang_code) {
                ...
                $notify_save_data[] = ['notify_id' => $user_notify->id, 'locale' => $lang_code, 'content' => __('logic.RECHARGE_USER_LEVEL_SUCCESS', ['name' => $level_name], $lang_code)];
            }
    ...
    public function addRechargeRebate(User $user, int $user_level)
    {
       ...
       // 一级返利
        ...
        if($p1_user){
             $p1_level = $this->container->get(UserMemberDAO::class)->getUserMaxLevel($p1_user->id);
            if($user_level>=$p1_level && $user_level!=8){ //体验会员不包含
                $user_level = $p1_level;
            }
            //echo '一级等级'.$p1_level.' 当前开通等级'.$user_level.'对应等级金额'.$level_list[$user_level]['price'];
            if (($p1_rebate =
                $level_list[$user_level]['recharge_level_rebate']['p_one_rebate'] ?? 0) > 0) {
                $u1_level_price = $level_list[$user_level]['price']??0;
                $a1_rebate  = $u1_level_price *$p1_rebate/100; 
                ...
                $this->container->get(UserBillDAO::class)->create([
                    ...
                    'after_balance'  => $p1_user->balance + $a1_rebate,
                    'low_id'         => $user->id,
                    'rebate_user_level'=>$user_level,
                    'rebate_level'   => 1,
                    'rebate_val'     => $p1_rebate
                ]);
    
                $p1_user->increment('balance',$a1_rebate);
            }
        }
        ...
        if($p2_user){
            $p2_level = $this->container->get(UserMemberDAO::class)->getUserMaxLevel($p2_user->id);
            if($user_level>=$p2_level){
                $user_level = $p2_level;
            }
            if ( ($p2_rebate = $level_list[$user_level]['recharge_level_rebate']['p_two_rebate'] ?? 0) > 0) {
                $u2_level_price = $level_list[$user_level]['price']??0;
                $a2_rebate = $u2_level_price*$p2_rebate/100;
                ...
                $this->container->get(UserBillDAO::class)->create([
                    ...
                    'after_balance'  => $p2_user->balance + $a2_rebate,
                    'low_id'         => $user->id,
                    'rebate_user_level'=>$user_level,
                    'rebate_level'   => 2,
                    'rebate_val'     => $p2_rebate
                ]);
    
                $p2_user->increment('balance', $a2_rebate);
            }
        }
        ...
        if($p3_user){
             $p3_level = $this->container->get(UserMemberDAO::class)->getUserMaxLevel($p3_user->id);
            if($user_level>=$p3_level){
                $user_level = $p3_level;
            }
            if (($p3_rebate = $level_list[$user_level]['recharge_level_rebate']['p_three_rebate'] ?? 0) > 0) {
                $u3_level_price = $level_list[$user_level]['price']??0;
                $a3_rebate = $u3_level_price*$p3_rebate/100;
                ...
                $this->container->get(UserBillDAO::class)->create([
                    ...
                    'after_balance'  => $p3_user->balance + $a3_rebate,
                    'low_id'         => $user->id,
                    'rebate_user_level'=>$user_level,
                    'rebate_level'   => 3,
                    'rebate_val'     => $p3_rebate
                ]);
    
                $p3_user->increment('balance', $a3_rebate);
            }
        }
    }
}