...
class AdminUserDAO extends Base
{
     ...
     * @return mixed
     */
    public function firstByPhone(string $phone, string $country_code): ?AdminUser
    {
        return AdminUser::query()->where('phone', $phone)->where('country_code', $country_code)->first();
    }
    ...
    {
        $admin = JwtInstance::instance()->build()->getUser();
        ...
        // 禁止操作上级
        /* if ($user->agent_level <= $admin->agent_level) {
            $this->error('logic.DO_NOT_OPERATE_SUPERIOR');
        }
        if ($data['agent_level'] <= $admin->agent_level) {
            $this->formError([
                'agent_level' => 'logic.AGENT_LEVEL_ERROR'
            ]);
        } */
    ...
    public function create(array $data): ?AdminUser
    {
        ...
        // 检查代理等级
        /* if ($data['agent_level'] <= $user->agent_level) {
            $this->formError([
                'agent_level' => 'logic.AGENT_LEVEL_ERROR'
            ]);
        } */
