...
class UserService extends Base
{
     ...
     */
    public function loginHandle(string $username, string $password, string $googleCode): ?AdminUser
    {
        // 普通账号
        if (!$user = $this->container->get(AdminUserDAO::class)->firstByUsername($username)) {
            $this->error('logic.USER_NOT_FOUND');
        }

        // 检测谷歌验证码
        if ($user->google_auth && $user->google_auth->is_enable) {
            $ga = new PHPGangsta_GoogleAuthenticator();
            if (!$checkResult = $ga->verifyCode($user->google_auth->secret, $googleCode, 2)) {
                $this->error('logic.GOOGLE_AUTH_VERIFY_CODE_ERROR');
            }
        }
