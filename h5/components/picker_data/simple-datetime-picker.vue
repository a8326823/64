      ...
      <view class="wrap" :class="{ show: open }">
         <view class="picker-header" @touchmove.stop.prevent catchtouchmove="true">
			 <view class="picker_button">
				 <text @click="open = false">{{i18n[0]}}</text>
				 <text @click="_onSubmit">{{i18n[1]}}</text>
			 </view>
         </view>
...
export default {
   ...
   methods: {
      ...
      initYears() {
         let years = [];
         for (let i = this.startear; i <= this.endYear; i++) {
            years.push(i);
            if (this.currentDate.getFullYear() === i) {
               this.$set(this.value, 0, i - this.startear);
            }
         }
         this.years = years;
      },
...
.picker_button{ display: flex;justify-content: space-between;border-bottom: 1px solid #ececec;width: 100%;
