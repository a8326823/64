<template>
	<view class="header_top pageback">
		<public-head title="奖励收益"></public-head>
		<view class="record_content">
			<view class="recordlist">
				<mescroll-body ref="mescrollRef" @init="mescrollInit" @down="downCallback" @up="upCallback" :up="upOption">
					<view class="item" v-for="(item,index) in dataList" :key="index">
						<view class="left">
							{{item.price}}{{item.sysmbol}}  121231545
						</view>
						<view class="right">
							<text :class="item.type == 1 ? 'color_red' : 'color_green'">{{item.profit}}</text> 
							<view class="date">
								{{item.time}}
							</view>
						</view>
...
export default {
	...
	data() {
		return {
			...
			dataList:[{price:"100",sysmbol:"$",type:0,time:"2012/12/12 03/12",profit:"首冲"},{price:"100",sysmbol:"U",type:1,time:"2012/12/12 03/12",profit:"返利"},
					  {price:"100",sysmbol:"$",type:2,time:"2012/12/12 03/12",profit:"返利"},{price:"100",sysmbol:"U",type:0,time:"2012/12/12 03/12",profit:"首冲"}]
		};
	},
	computed:{
		i18n(){
	...
	methods: {
		loadjson(page){
			...
			let that = this;
			this.$http.requestajx('user_withdrawal1','get',{
				page: pageNum
			}).then(function(res){
				// 接口返回的当前页数据列表 (数组)
